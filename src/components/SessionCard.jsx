 ...
 *   isFollowing    – whether current user follows session.user_id (pass null to hide follow button)
 *   onFollowChange – (targetUserId, nowFollowing) => void  (called optimistically)
 */
export default function SessionCard({ session, userId, isFollowing, onFollowChange }) {
  ...
  async function handleFollow() {
    ...
    onFollowChange(session.user_id, willFollow) // optimistic
    ...
    try {
      ...
      onFollowChange(session.user_id, !willFollow) // revert
    } finally {
    ...
    <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden hover:border-stone-300 transition">
      ...
      {session.ai_feedback && (
        <div className="border-t border-stone-100">
          ...
          >
            <span className="text-xs font-semibold text-amber-600 uppercase tracking-wide">AI Coaching</span>
